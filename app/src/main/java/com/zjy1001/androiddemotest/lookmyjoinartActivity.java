...
public class lookmyjoinartActivity extends AppCompatActivity implements HttpCallBack{
    @BindView(R.id.main_toolbarss)
    Toolbar ptoolbar;
    @BindView( R.id.toolbar_titless )
    TextView tv1;
    @BindView( R.id.myjoinart_list )
    ListView lv;
    List<Joarticle> list;
    String userid;
   ...
   private String path="http://10.0.2.2:8080/EIP/MyJoinArtController/lookmyjoinart.action";
   //private String path="http://192.168.43.216:8080/EIP/MyJoinArtController/lookmyjoinart.action";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate( savedInstanceState );
        setContentView( R.layout.activity_lookmyjoinart );
        ButterKnife.bind(this);
        final Intent intent=getIntent();
        userid=intent.getStringExtra( "userid" );//登录者id
        ptoolbar.setTitle("我收藏的文章");
        ...
        //区分请求来源，加入标杆
        map.put( "flag","lookmyjoinart" );
        map.put( "userid",userid );
        //将map类型转成固定格式的字符串
        String string= Util.getPair(map);
        HttpTask task=new HttpTask(this);
        //设置数据回调
        task.setCallback(lookmyjoinartActivity.this);
        //开始执行请求
    ...
    public void callback(String respone) {
        if (respone != null) {
            ...
            if (part1.equals( "objectlookmyjoinart" )){
                //将jsonarray类型的数据转化成集合
                Gson gson = new Gson();
                Type type = new TypeToken<List<Joarticle>>() {
                }.getType();
                ...
                lv.setOnItemClickListener( new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        ...
                        aitent = aitent.setClass( lookmyjoinartActivity.this, lookmyjoinartActivity_item.class );
